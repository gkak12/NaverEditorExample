 		...
 		try{
	    	...
	    	for (var i = 0; i < aTemp.length ; i++){
	    		if( !!aTemp[i] && aTemp[i] != "" && aTemp[i].indexOf("=") > 0){
	    			...
	    			htTemp[aSubTemp[0]] = aSubTemp[1].replaceAll("\n", "");
	    		}
	 		}
 		}catch(e){}
	...
	function onAjaxError (){
 	...
 	function callFileUploader (){
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 			sUrl  : "file_uploader.jsp",	//location.href.replace(/\/[^\/]*$/, '') + '/file_uploader.php',	//샘플 URL입니다.
 	        sCallback : "/smarteditor2/sample/photo_uploader/callback.html",	//location.href.replace(/\/[^\/]*$/, '') + '/callback.html',	//업로드 이후에 iframe이 redirect될 콜백페이지의 주소
